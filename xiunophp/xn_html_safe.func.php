	...
	public function getXHTML() {
       ...
       private function _writeAttrs($attrs, $tagname) {
		...
		foreach($attrs as $name => $value) {
			...
			// strictly block javascript
			if (preg_match('/(j\s*a\s*v\s*a\s*s\s*c\s*r\s*i\s*p\s*t|v\s*b\s*s\s*c\s*r\s*i\s*p\s*t|d\s*a\s*t\s*a\s*:)/i', $tempval)) {
				// but allow data:image
				if (!preg_match('/^d\s*a\s*t\s*a\s*:\s*i\s*m\s*a\s*g\s*e\s*\//i', $tempval)) {
					$value = '';
					$tempval = '';
				}
			}
...
/*function xn_html_safe($doc, $arg = array()) {
	...
	$pattern = array (
		...
		'url'=>'#^(https?://|//|\./|\.\./|/|[\w\-]+)[^\'"\\\\<>:\s]*$#is', // strictly prevent javascript:
