...
// 安装初始化检测,放这里
is_file(APP_PATH.'conf/install.lock') AND message(0, jump(lang('installed_tips'), '../'));
...
if(empty($action)) {
	...
	if($method == 'GET') {
		...
		$type = in_array(param('type'), array('mysql', 'pdo_mysql')) ? param('type') : 'pdo_mysql';
		$engine = in_array(param('engine'), array('myisam', 'innodb')) ? param('engine') : 'innodb';
		$host = param('host', '', FALSE);
		$name = param('name', '', FALSE);
		$user = param('user', '', FALSE);
		$password = param('password', '', FALSE);
		$force = param('force');

		$adminemail = param('adminemail', '', FALSE);
		$adminuser = param('adminuser', '', FALSE);
		$adminpass = param('adminpass', '', FALSE);
		...
		if(!preg_match('/^[a-zA-Z0-9_\-\.\:]+$/', $host)) {
			message('host', '数据库主机格式不正确 / Database host is invalid');
		}
		if(!preg_match('/^[a-zA-Z0-9_]+$/', $name)) {
			message('name', '数据库名格式不正确 / Database name is invalid');
		}
		if(!preg_match('/^[a-zA-Z0-9_]+$/', $user)) {
			message('user', '数据库用户名格式不正确 / Database user is invalid');
		}
		if(strlen($adminpass) < 6) {
			message('adminpass', '管理员密码不能少于6位 / Admin password must be at least 6 characters');
		}
		...
		// 安装成功，写入 lock 文件
		file_put_contents(APP_PATH.'conf/install.lock', 'locked');
