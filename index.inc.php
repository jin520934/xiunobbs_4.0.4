...
// CSRF Defense for POST requests
if($_SERVER['REQUEST_METHOD'] == 'POST') {
	$referer = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : '';
	$host = isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : '';
	if($referer) {
		$ref_host = parse_url($referer, PHP_URL_HOST);
		if($ref_host && $host && $ref_host != $host && $ref_host != str_replace('www.', '', $host) && 'www.'.$ref_host != $host) {
			message(-1, 'Invalid Referer, CSRF detected.');
		}
	} else {
		// Strict mode: deny POST without referer (might break some clients, but secure)
		message(-1, 'Missing Referer, CSRF detected.');
	}
}
